...
export default function TheHub(): React.JSX.Element {
  ...
  async function sendMessage() {
    ...
    try {
      ...
      const response = await chat([...history, { role: 'user', content: userMsg }], model, ollamaHost)
  ...
  const visibleMessages = query
    ? messages.filter((m) => m.message.toLowerCase().includes(query.toLowerCase()) || m.from_agent.toLowerCase().includes(query.toLowerCase()))
    : messages
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div>
            ...
            <div style={{ fontSize: 11, color: 'var(--gh-text-4)', marginTop: 2 }}>
              {query ? `${visibleMessages.length} messages matching "${query}"` : 'Agent communication channel'}
            </div>
          </div>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: 'var(--sp-4) var(--sp-6)', display: 'flex', flexDirection: 'column', gap: 'var(--sp-3)' }}>
        {visibleMessages.length === 0 ? (
          <div className="empty-state" style={{ flex: 1 }}>
            ...
            <span className="empty-text">{query ? `No messages matching "${query}"` : 'No messages yet. Start a conversation with Motu below.'}</span>
          </div>
          ...
          return (
            <div key={msg.id} style={{ display: 'flex', gap: 'var(--sp-3)', flexDirection: isUser ? 'row-reverse' : 'row', alignItems: 'flex-start' }}>
              <div style={{ width: 30, height: 30, borderRadius: 'var(--r-md)', background: isUser ? 'var(--gh-elevated)' : `linear-gradient(135deg, ${agentColor}33, ${agentColor}11)`, border: `1px solid ${isUser ? 'var(--gh-border)' : agentColor + '44'}`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 14, flexShrink: 0, boxShadow: isUser ? 'none' : `0 0 10px ${agentColor}22` }}>
        ...
        {sending && (
          <div style={{ display: 'flex', gap: 'var(--sp-3)', alignItems: 'flex-start' }}>
            <div style={{ width: 30, height: 30, borderRadius: 'var(--r-md)', background: 'linear-gradient(135deg, rgba(88,166,255,0.2), rgba(88,166,255,0.05))', border: '1px solid var(--blue-border)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 14 }}>🤖</div>
            <div style={{ background: 'rgba(22,27,34,0.8)', border: '1px solid var(--gh-border)', borderRadius: 'var(--r-sm) var(--r-lg) var(--r-lg) var(--r-lg)', padding: 'var(--sp-3) var(--sp-4)', fontSize: 12.5, color: 'var(--gh-text-3)', backdropFilter: 'blur(8px)' }}>
              <span style={{ animation: 'pulse 1s infinite' }}>Motu is thinking...</span>
            </div>
          </div>
        )}
        <div ref={bottomRef} />
      ...
      {!query && (
        <div style={{ padding: 'var(--sp-4) var(--sp-6)', borderTop: '1px solid var(--gh-border)', flexShrink: 0, background: 'rgba(13,17,23,0.3)' }}>
          <div style={{ display: 'flex', gap: 'var(--sp-3)', alignItems: 'flex-end' }}>
            ...
            />
            <button className="btn btn-orange" style={{ height: 44, paddingLeft: 'var(--sp-5)', paddingRight: 'var(--sp-5)' }} onClick={sendMessage} disabled={sending || !input.trim()}>
              {sending ? '...' : 'Send'}
            </button>
          </div>
          <div style={{ fontSize: 10, color: 'var(--gh-text-4)', marginTop: 'var(--sp-2)' }}>
            Enter to send · Shift+Enter for new line · Connected to {ollamaHost}
          </div>
        </div>
      )}
    </div>
